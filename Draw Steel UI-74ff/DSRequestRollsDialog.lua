...
function RollCheck:GetModifiers(creature)
	...
	if self.type == "test_power_roll" then
		local result = creature:GetModifiersForPowerRoll(self:GetRoll(creature), self.type, {attribute = self.id})
        local skill = self:GetSkill()
                        ...
                        for i=1,#GameSystem.TierNames do
                            ...
                            local input = gui.Input{
                                ...
                                update = function(element)
                                    local powerTable = PowerRollTableGroup.GetPowerTable(g_selectedPowerRoll:Get())
