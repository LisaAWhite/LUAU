...
function GameHud.CreateRollDialog(self)
	...
	CalculateRollText = function(calculationOptions)
		...
		if creature and isd20roll then
			for i,mod in ipairs(enabledModifiers) do
				if not mod.modFromTarget then
					roll = mod.modifier:ModifyD20RollEarly(mod.context, creature, rollSubtype, roll)
				end
			end
		end
		...
		if creature then
			for i,mod in ipairs(enabledModifiers) do
				...
				if isd20roll then
					if mod.modFromTarget then
						roll = mod.modifier:ModifyAttackAgainstUs(mod.context, targetCreature, creature, roll)
					else
						roll = mod.modifier:ModifyD20Roll(mod.context, creature, rollSubtype, roll)
					end
				end
    ...
    RecalculateMultiTargets = function()
        if m_multitargets == nil or rollProperties == nil or #m_multitargets <= 1 then
            return
        end
