...
function CharacterModifier:ConsumeResourceInternal(creature, modContext)
    ...
    local costType = self:try_get("resourceCostType", "none")
    if costType ~= "none" then
        local resourcesAvailable = creature:GetHeroicOrMaliceResourcesAvailable()
        local charges = self:try_get("_tmp_symbols", {}).charges or 1
        local cost = tonumber(self:try_get("resourceCostAmount", 1))*charges
		local resourcesTable = dmhub.GetTable("characterResources")
		local resourceInfo = resourcesTable[creature.resourceid]
...
function CharacterModifier:HasResourcesAvailable(creature)
    ...
    if costType ~= "none" then
        local resourcesAvailable = creature:GetHeroicOrMaliceResourcesAvailable()
        local cost = tonumber(self:try_get("resourceCostAmount", 1))
...
function CharacterModifier:DescribeResourceAvailability(creature, charges)
    ...
    if costType ~= "none" then
        local resourcesAvailable = creature:GetHeroicOrMaliceResourcesAvailable()
        local cost = tonumber(self:try_get("resourceCostAmount", 1))*charges
        return string.format("%d/%d %s", cost, resourcesAvailable, creature:ResourceName())
    end
