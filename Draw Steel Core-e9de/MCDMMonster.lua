...
function monster:FillFreeStrikes(options, result)
    ...
    if meleeFreeStrike ~= nil then
        ...
        ability.behaviors[1].roll = tostring(self:OpportunityAttack()) .. "*Charges"
        ability.behaviors[1].damageType = damageType

        if damageType == "normal" then
            ability.description = string.format("%s damage", ability.behaviors[1].roll)
        else
            ability.description = string.format("%s %s damage", ability.behaviors[1].roll, damageType)
        end
