...
creature.RegisterSymbol{
	symbol = "flankedby",
	lookup = function(c)
		return function(otherCreature)
			local tok = dmhub.LookupToken(otherCreature)
			if tok ~= nil then
				return c:FlankedBy(tok)
			end
	...
	help = {
		name = "Flanked By",
		type = "function",
		desc = "Given another creature, tells us if this creature is flanked by that creature.",
		seealso = {},
	},
}
...
function creature:GetFlankingTokens()
	local token = dmhub.LookupToken(self)
	...
	local adjacentEnemies = GetEnemiesAdjacentToToken(token)
