...
UnitTest(function()
    UnitTestEq(MonsterNameToMatchPattern("Goblin"), "([a-z0-9' ]{3,20}|goblin|the goblin)")
    UnitTestEq(MonsterNameToMatchPattern("Goblin Boss"), "([a-z0-9' ]{3,20}|goblin boss|goblin|the goblin|boss|the boss)")
...
local function CreateMatchPattern(monsterName, pattern)
    ...
    while matchGroup ~= nil and n > 0 do
        pattern = regex.ReplaceAll(pattern, "<<" .. matchGroup["identifier"] .. ">>", "(?<" .. matchGroup["identifier"] .. ">[0-9d]+)")
...
function CharacterFeature:MatchMCDMMonsterTrait(bestiaryEntry, name, description)
    ...
    if matches ~= nil then
        ...
        for key,val in pairs(matches) do
            ...
            local value = val
            if key == "surges" or key == "shields" then
                local _, count = val:gsub(":", "")
                value = tostring(count/2)
            end

            MCDMUtils.DeepReplace(clone, keyPattern, value)
        end
