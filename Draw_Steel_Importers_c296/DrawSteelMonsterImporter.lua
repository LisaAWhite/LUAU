...
MCDMImporter.ParseMonsterAbility = function(bestiaryEntry, lines, knownAbilities)

    import:Log("--")

    local headerLine = lines[1]

    local abilityHeaderMatch = regex.MatchGroups(headerLine, "^<b>(?<name>[^<]+)</b>\\s*\\((?<action>Action|Triggered Action|Maneuver|Free Action|Free Triggered Action|Villain Action 1|Villain Action 2|Villain Action 3)\\).*?(?<signature>Signature)?((?<vp>[0-9]+) (VP|Malice))?$")
    ...
    for i=2,#lines do
        ...
        elseif powerRollLine3 ~= nil then
            ...
            while trim(remaining) ~= "" and count < 5 do
                ...
                --first try to match effect which would take the entire line.
    ...
    print("TARGET:: PARSING ABILITY", newAbility.name)
    ...
    local numberedTargetsMatch = regex.MatchGroups(target, "(?<number>[0-9]+|One|Two|Three|Four|Five|Six|Seven|Eight|Nine|Ten) (?<type>creature|creatures|ally|allies|enemy|enemies)( or objects?)?(of weight (?<weightRequirement>[0-9]+) or lower)?( per minion)?")
            ...
            if ability.roll == nil or ability.tiers == nil or #ability.tiers ~= 3 then
                ...
                if abilityPowerRoll == nil then
                    ...
                    for _,behavior in ipairs(abilityPowerRoll.behaviors) do
                        ...
                        print("Behavior:: for", bestiaryEntry.name, "have", b)
                        newAbility.behaviors[#newAbility.behaviors+1] = b
                    end
...
MCDMImporter.ImportText = function(importer, text)
    ...
    while i <= #lines do
        ...
        elseif monsterHeadingMatch == nil then
            ...
            print("ENDLINE:: MONSTER", monsterHeadingMatch.name, "ENDLINE", endLine)
            ...
            while attrMatch ~= nil and i < endLine do
                ...
                local abilityHeaderMatch = regex.MatchGroups(sline, "^<b>(?<name>[^<]+)</b>\\s*\\((?<type>Action|Triggered Action|Maneuver|Free Action|Free Triggered Action|Villain Action 1|Villain Action 2|Villain Action 3)\\).*(?<signature>Signature)?((?<vp>[0-9]+) (VP|Malice))?$")
                if abilityHeaderMatch ~= nil then
                    local entries = {}
                    ...
                    --ability goes until next whitespace.
                    while i <= #lines do
                        sline = lines[i] or ""
                        i = i+1
                ...
                elseif regex.MatchGroups(sline, monsterHeaderPattern) ~= nil then
                    print("MONSTER:: monster", monsterHeadingMatch.name, "NEXT MONSTER")
                    --we've hit the next monster.
                    i = i-1
                    break
                elseif lastWhitespace == 1 and regex.MatchGroups(sline, "^\\s*<b>[^<]+</b>\\s*$") ~= nil then
                    print("MONSTER:: monster", monsterHeadingMatch.name, "TRAIT")
                    --trait.
                    local traitMatch = regex.MatchGroups(sline, "^\\s*<b>(?<name>[^<]+)</b>\\s*$")
                    local traitText = ""
                ...
                elseif whitespaceLinesRun == 0 then
                    print("MONSTER:: monster", monsterHeadingMatch.name, "UNRECOGNIZED WITH WHITESPACE", lastWhitespace)
            ...
            if error ~= nil then
                ...
                print("MONSTER:: PARSED AND ADDING")
                ...
                if bestiaryEntry ~= nil and ((not bestiaryEntry.properties:has_key("import")) or (not bestiaryEntry.properties.import.override)) then
                    ...
                    for _,abilityLines in ipairs(abilitiesEntries) do
                        local ability = MCDMImporter.ParseMonsterAbility(bestiaryEntry, abilityLines, knownAbilities)
                        if ability ~= nil then
